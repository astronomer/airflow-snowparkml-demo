...
--------------------------------------
with 
customer_conversions as (select * from  stg_customer_conversions),
sessions as (select * from  stg_sessions),

sessions_before_conversion as (
    select
        *,
        count(*) over (partition by customer_id) as total_sessions,
        row_number() over (partition by customer_id order by sessions.started_at) as session_index
    from sessions
...
create table attribution_touches as select * from with_points;